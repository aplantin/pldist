...
$$ \mbox{Qualitative:  } \qquad D_{AB} = 1 - \frac{1}{m} \sum_k I(d_k^A = d_k^B) I(d_k^A \neq 0) $$ 
and 
$$ \mbox{Quantitative:  } \qquad  D_{AB} = 1 - \frac{2}{m} \sum_k \min(|d_k^A|, |d_k^B|) \, I\lbrace \mbox{sgn}(d_k^A) = \mbox{sgn}(d_k^B) \rbrace. $$ 
Similarly, both qualitative and quantitative longitudinal versions may be calculated as 
$$ D_{AB} = 1 - \frac{1}{m} \sum_k \min(d_k^A, d_k^B). $$ 
