...
- Transformed data (accessible by res$tsf.data), a list with components 
    + dat.binary: data matrix following qualitative/binary transformation (as described in the later section [Transformations and Dissimilarities]) 
    + dat.quant: data matrix following quantitative transformation (again, as described later) 
...
The paired and longitudinal UniFrac family distances/dissimilarities, collectively referred to as LUniFrac, may be accessed using the `LUniFrac` function, shown below, or using the overall distance function `pldist`, demonstrated in the next section. 
...
The primary, general form of the distance function is `pldist`. 
